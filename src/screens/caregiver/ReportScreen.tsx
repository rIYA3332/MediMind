...
const MedStackBar: React.FC<{ taken: number; partial: number; missed: number; total: number }> = ({
  ...
  return (
    <View>
      <View style={{ flexDirection: 'row', height: 12, borderRadius: 6, overflow: 'hidden', marginBottom: 8 }}>
...
const HealthScoreCard: React.FC<{
  ...
  const label = score >= 80 ? 'Good' : score >= 60 ? 'Fair' : score >= 40 ? 'Needs Attention' : 'Critical';

  return (
    <View style={[SS.scoreCard, { borderColor: color + '33' }]}>
      <View style={SS.scoreLeft}>
        <DonutRing pct={score} size={88} color={color} label={`${score}`} sublabel="/ 100" />
      </View>
      <View style={SS.scoreRight}>
        <Text style={[SS.scoreLabel, { color }]}>{label}</Text>
        <Text style={SS.scoreHint}>Health score this period</Text>
        <View style={{ gap: 5, marginTop: 10 }}>
          <MiniBar value={adherencePct}   max={100} color="#00b894" label="Medications"  suffix="%" />
          <MiniBar value={moodScore}      max={100} color="#74b9ff" label="Mood"         suffix="%" />
          <MiniBar value={vitalStability} max={100} color="#6c5ce7" label="Vitals"       suffix="%" />
        </View>
        {criticalRisks > 0 && (
          <View style={SS.criticalNote}>
            <Text style={SS.criticalNoteText}>
              {criticalRisks} critical alert{criticalRisks > 1 ? 's' : ''} need attention
            </Text>
          </View>
        )}
      </View>
...
const SS = StyleSheet.create({
  scoreCard:        { flexDirection: 'row', backgroundColor: colors.white, borderRadius: 14, padding: 16, borderWidth: 1, marginBottom: 14 },
  scoreLeft:        { marginRight: 16, justifyContent: 'center' },
  scoreRight:       { flex: 1, justifyContent: 'center' },
  scoreLabel:       { fontSize: 18, fontWeight: '800', marginBottom: 1 },
  scoreHint:        { fontSize: 11, color: colors.textSecondary },
  criticalNote:     { marginTop: 8, backgroundColor: '#fff0f0', borderRadius: 8, padding: 6, borderWidth: 1, borderColor: '#ff7675' },
  criticalNoteText: { fontSize: 11, color: '#c0392b', fontWeight: '600' },
});

// ── Note pill ──────────────────────────────────────────────────────────────────
const NotePill: React.FC<{ text: string; type: 'good' | 'warn' | 'bad' | 'info' }> = ({ text, type }) => {
  const cfg = {
    good: { bg: '#f0fdf4', border: '#86efac', text: '#166534' },
    warn: { bg: '#fffbeb', border: '#fde68a', text: '#92400e' },
    bad:  { bg: '#fff1f2', border: '#fecdd3', text: '#9f1239' },
    info: { bg: '#eff6ff', border: '#bfdbfe', text: '#1e40af' },
  }[type];
  return (
    <View style={[NP.pill, { backgroundColor: cfg.bg, borderColor: cfg.border }]}>
      <Text style={[NP.text, { color: cfg.text }]}>{text}</Text>
    </View>
  );
};
const NP = StyleSheet.create({
  pill: { padding: 11, borderRadius: 10, borderWidth: 1, marginBottom: 6 },
  text: { fontSize: 13, lineHeight: 19 },
});

// ── Collapsible Medication Row ─────────────────────────────────────────────────
        ...
        <View style={CMR.detail}>
          ...
          {med.adherence_pct < 70 && (
            <View style={CMR.tipBox}>
              <Text style={CMR.tipText}>Consider adjusting the reminder time or checking for any side effects.</Text>
            </View>
          )}
        </View>
      )}
...
const CMR = StyleSheet.create({
  ...
  tipBox:   { marginTop: 10, backgroundColor: '#fffbeb', borderRadius: 8, padding: 10, borderWidth: 1, borderColor: '#fde68a' },
  tipText:  { fontSize: 12, color: '#92400e', lineHeight: 18 },
});

// ── Vital Reading Card ─────────────────────────────────────────────────────────
const VitalCard: React.FC<{ vital: VitalAnalysis }> = ({ vital }) => {
  const tc = vital.regression.trend === 'rising' ? '#e17055' : vital.regression.trend === 'falling' ? '#0984e3' : '#00b894';
  const tl = vital.regression.trend === 'rising' ? '↑ Rising' : vital.regression.trend === 'falling' ? '↓ Falling' : '→ Stable';
  ...
  return (
    <View style={VC.card}>
      ...
      <View style={VC.topRow}>
        <View style={[VC.iconWrap, { backgroundColor: vColor + '15' }]}>
          <Text style={{ fontSize: 18 }}>{healthIcons[vital.log_type] || '📊'}</Text>
        </View>
        <View style={{ flex: 1, marginLeft: 10 }}>
          <Text style={VC.name}>{vital.label}</Text>
          <Text style={{ fontSize: 11, color: colors.textSecondary }}>
            {vital.stats.count} reading{vital.stats.count !== 1 ? 's' : ''}
          </Text>
        </View>
        <View style={[VC.trendPill, { backgroundColor: tc + '15', borderColor: tc + '40' }]}>
          <Text style={[VC.trendTxt, { color: tc }]}>{tl}</Text>
        </View>
      </View>

      {/* Numbers */}
      <View style={VC.numRow}>
        <View style={[VC.latestBox, { borderColor: vColor + '40', backgroundColor: vColor + '08' }]}>
          <Text style={[VC.latestNum, { color: vColor }]}>{vital.stats.latest}</Text>
          <Text style={VC.latestUnit}>{vital.unit}</Text>
          <Text style={VC.latestLbl}>Latest</Text>
        </View>
        <View style={VC.divider} />
        <View style={VC.numItem}>
          <Text style={VC.numVal}>{vital.stats.avg}</Text>
          <Text style={VC.numLbl}>Average</Text>
        </View>
        <View style={VC.divider} />
        <View style={VC.numItem}>
          <Text style={[VC.numVal, { color: '#00b894' }]}>{vital.stats.min}</Text>
          <Text style={VC.numLbl}>Low</Text>
        </View>
        <View style={VC.divider} />
        <View style={VC.numItem}>
          <Text style={[VC.numVal, { color: '#e17055' }]}>{vital.stats.max}</Text>
          <Text style={VC.numLbl}>High</Text>
        </View>
      </View>

      {vital.readings.length > 1 && (
        <SparkBars values={vital.readings.slice(-10).map(r => r.numeric)} color={vColor} height={34} />
      )}

      {changeAbs > 0 && vital.regression.trend !== 'stable' && (
        <Text style={{ fontSize: 11, color: tc, marginTop: 2 }}>
          {vital.regression.trend === 'rising' ? 'Increasing' : 'Decreasing'} ~{changeAbs} {vital.unit} per week
        </Text>
      )}
    </View>
  );
...
const VC = StyleSheet.create({
  ...
  trendPill: { paddingHorizontal: 9, paddingVertical: 4, borderRadius: 20, borderWidth: 1 },
  trendTxt:  { fontSize: 11, fontWeight: '700' },
  numRow:    { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f8f9fb', borderRadius: 10, padding: 10, marginBottom: 4 },
  latestBox: { alignItems: 'center', flex: 1.2, borderWidth: 1, borderRadius: 8, paddingVertical: 6 },
  latestNum: { fontSize: 18, fontWeight: '800' },
  latestUnit:{ fontSize: 9, color: colors.textSecondary },
  latestLbl: { fontSize: 10, color: colors.textSecondary, marginTop: 2 },
  divider:   { width: 1, height: 32, backgroundColor: '#e5e7eb', marginHorizontal: 6 },
  numItem:   { flex: 1, alignItems: 'center' },
  numVal:    { fontSize: 14, fontWeight: '700', color: colors.textPrimary },
  numLbl:    { fontSize: 10, color: colors.textSecondary, marginTop: 2 },
});
...
const ReportScreen = ({ route, navigation }: any) => {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      else Alert.alert('Error', 'Cannot open PDF link.');
  ...
  const sev = (s: string) => s === 'critical'
    ? { bg: '#fff1f2', border: '#fecdd3', text: '#9f1239', label: 'Critical' }
    : s === 'danger'
    ? { bg: '#fffbeb', border: '#fde68a', text: '#92400e', label: 'High'     }
    : { bg: '#f0fdf4', border: '#bbf7d0', text: '#166534', label: 'Watch'    };
  ...
  // Plain-English notes (no AI buzzwords)
  const positives:   Array<string> = [];
  const concerns:    Array<string> = [];
  const suggestions: Array<string> = [];

  if (adherencePct >= 90) positives.push(`Medications taken consistently — ${adherencePct}% adherence this period.`);
  else if (adherencePct >= 70) concerns.push(`${missedMedDays} doses were missed (${adherencePct}% taken). Setting daily reminders may help.`);
  else concerns.push(`Medication adherence is low at ${adherencePct}%. ${missedMedDays} doses missed — please follow up.`);

  if (happyCount > 0 && happyCount >= negMoodCount) positives.push(`Mood has been mostly positive — ${happyCount} happy day${happyCount !== 1 ? 's' : ''} recorded.`);
  if (negMoodCount > 2) concerns.push(`${negMoodCount} days of sad, anxious, or lonely mood. More social contact may help.`);

  if (criticalRisks > 0) concerns.push(`${criticalRisks} critical health alert${criticalRisks !== 1 ? 's' : ''} — medical review needed promptly.`);
  if (dangerRisks > 0)   concerns.push(`${dangerRisks} elevated health reading${dangerRisks !== 1 ? 's' : ''} — worth discussing with a doctor.`);
  if (totalRisks === 0)  positives.push('No health alerts this period. Readings are within expected ranges.');

  if (risingVitals.length > 0)
    concerns.push(`${risingVitals.map(v => v.label).join(' and ')} ${risingVitals.length > 1 ? 'are' : 'is'} trending up — worth monitoring.`);
  if (stableVitals > 0)
    positives.push(`${stableVitals} reading${stableVitals !== 1 ? 's are' : ' is'} stable with no significant changes.`);

  if (healthLogs > 0) positives.push(`${healthLogs} health readings logged this period.`);
  else suggestions.push('No readings logged yet. Daily tracking gives a better picture of health.');

  if (adherence?.worst_med && adherence.worst_med.adherence_pct < 70)
    suggestions.push(`"${adherence.worst_med.title}" has the lowest adherence (${adherence.worst_med.adherence_pct}%). Try adjusting the reminder time.`);
  if (negMoodCount > 2)
    suggestions.push('Schedule a regular call or visit — social contact often improves mood significantly.');
  suggestions.push('Bring this report to the next doctor or clinic visit.');
  ...
  const renderSummary = () => (
    <>
      <HealthScoreCard
        score={healthScore} adherencePct={adherencePct}
        criticalRisks={criticalRisks} moodScore={moodScore} vitalStability={vitalStability}
      />

      {/* Quick numbers */}
      <View style={styles.statsGrid}>
        {[
          { label: 'Doses Taken',  value: `${takenMedDays}/${totalMedDays}`, color: adherenceClr  },
          { label: 'Readings',     value: `${healthLogs}`,                   color: '#6c5ce7'     },
          { label: 'Mood Logs',    value: `${moodTotal}`,                    color: '#74b9ff'     },
          { label: 'Alerts',       value: `${totalRisks}`,                   color: totalRisks > 0 ? '#e17055' : '#00b894' },
        ].map(s => (
      ...
      {/* Medications */}
      <Text style={styles.secHeader}>Medications</Text>
      <Card style={styles.card}>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 14 }}>
          <DonutRing pct={adherencePct} size={74} color={adherenceClr} label={`${adherencePct}%`} />
          <View style={{ flex: 1, marginLeft: 16 }}>
            ...
            {adherence?.worst_med && (
              <Text style={{ fontSize: 11, color: '#e17055', marginTop: 5 }}>
                Lowest: {adherence.worst_med.title} ({adherence.worst_med.adherence_pct}%)
              </Text>
            )}
          </View>
        </View>
        ...
        {adherence?.medications && adherence.medications.length > 0 && (
          <View style={{ marginTop: 14 }}>
            <TouchableOpacity style={styles.dropdownToggle} onPress={() => setMedBreakdownOpen(o => !o)} activeOpacity={0.75}>
              <Text style={styles.dropdownToggleLabel}>Per medication ({adherence.medications.length})</Text>
              <Text style={[styles.dropdownChevron, { transform: [{ rotate: medBreakdownOpen ? '90deg' : '0deg' }] }]}>›</Text>
            </TouchableOpacity>
            {medBreakdownOpen && (
              <View style={{ marginTop: 10 }}>
                {adherence.medications.map(med => <CollapsibleMedRow key={med.id} med={med} />)}
              </View>
            )}
          </View>
        )}
      ...
      {goodVitals.length > 0 && (
        <>
          <Text style={styles.secHeader}>Vital Readings</Text>
          {goodVitals.map(vital => <VitalCard key={vital.log_type} vital={vital} />)}
        </>
      )}
      ...
      {report && report.mood.length > 0 && (
        <>
          <Text style={styles.secHeader}>Mood</Text>
          <Card style={styles.card}>
            ...
            {negMoodCount > 2 && (
              <View style={[styles.alertPill, { marginTop: 10 }]}>
                <Text style={styles.alertPillTxt}>
                  {negMoodCount} difficult mood days this period. More social contact may help.
                </Text>
              </View>
            )}
          </Card>
      ...
      {aiInsights.length > 0 && (
        <>
          <Text style={styles.secHeader}>Observations</Text>
          <Card style={styles.card}>
            {aiInsights.map((note, i) => <NotePill key={i} text={note} type="warn" />)}
          </Card>
        </>
      )}
      ...
      {positives.length > 0 && (
        <>
          <Text style={styles.secHeader}>What's Going Well</Text>
          <Card style={styles.card}>
            {positives.map((p, i) => <NotePill key={i} text={p} type="good" />)}
          </Card>
        </>
      )}
      ...
      {concerns.length > 0 && (
        <>
          <Text style={styles.secHeader}>Things to Follow Up</Text>
          <Card style={styles.card}>
            {concerns.map((c, i) => <NotePill key={i} text={c} type={c.includes('critical') ? 'bad' : 'warn'} />)}
          </Card>
        </>
      )}

      <Text style={styles.secHeader}>Suggestions</Text>
      <Card style={styles.card}>
        {suggestions.map((s, i) => <NotePill key={i} text={s} type="info" />)}
      </Card>

      {report && report.risks.length > 0 && (
        <>
          <Text style={styles.secHeader}>Health Alerts ({report.risks.length})</Text>
          {report.risks.map((risk, i) => {
            const sv = sev(risk.severity);
            return (
              <View key={i} style={[styles.riskCard, { borderLeftColor: sv.border, backgroundColor: sv.bg }]}>
                <View style={[styles.sevTag, { backgroundColor: sv.border + '30' }]}>
                  <Text style={[styles.sevTagTxt, { color: sv.text }]}>{sv.label}</Text>
                </View>
  ...
  const renderMedications = () => (
    <>
      <Text style={styles.secHeader}>Medication Adherence</Text>
      <Card style={styles.card}>
        <View style={{ alignItems: 'center', marginBottom: 16 }}>
          <DonutRing pct={adherencePct} size={108} color={adherenceClr} label={`${adherencePct}%`} sublabel="adherence" />
          <View style={[styles.badge, { backgroundColor: adherenceClr + '18', borderColor: adherenceClr, marginTop: 10 }]}>
            <Text style={[styles.badgeText, { color: adherenceClr }]}>{adherenceLabel}</Text>
          </View>
        </View>
        <MedStackBar taken={takenMedDays} partial={partialMedDays} missed={missedMedDays} total={totalMedDays} />
        <View style={styles.statsGrid3}>
          <View style={styles.statBox3}><Text style={[styles.statValue3, { color: '#00b894' }]}>{takenMedDays}</Text><Text style={styles.statLabel3}>Taken</Text></View>
          {partialMedDays > 0 && <View style={styles.statBox3}><Text style={[styles.statValue3, { color: '#fdcb6e' }]}>{partialMedDays}</Text><Text style={styles.statLabel3}>Partial</Text></View>}
          <View style={styles.statBox3}><Text style={[styles.statValue3, { color: '#ff7675' }]}>{missedMedDays}</Text><Text style={styles.statLabel3}>Missed</Text></View>
          <View style={styles.statBox3}><Text style={[styles.statValue3, { color: colors.primary }]}>{totalMedDays}</Text><Text style={styles.statLabel3}>Total</Text></View>
        </View>
      </Card>

      {adherence?.medications && adherence.medications.length > 0 && (
        <>
          <Text style={styles.secHeader}>Breakdown by Medication</Text>
          {adherence.medications.map(med => <CollapsibleMedRow key={med.id} med={med} />)}
        </>
      )}

      {adherence?.worst_med && adherence.worst_med.adherence_pct < 80 && (
        <View style={[styles.riskCard, { borderLeftColor: '#fdcb6e', backgroundColor: '#fffbeb', marginTop: 4 }]}>
          <View style={[styles.sevTag, { backgroundColor: '#fde68a' }]}>
            <Text style={[styles.sevTagTxt, { color: '#92400e' }]}>Note</Text>
          </View>
          <Text style={styles.riskMsg}>
            "{adherence.worst_med.title}" — {adherence.worst_med.adherence_pct}% adherence
            ({adherence.worst_med.days_missed} missed out of {adherence.worst_med.days_due} days).
            Consider reviewing the reminder schedule.
          </Text>
        </View>
      )}
    </>
      ...
      {!report?.healthLogs.length ? (
        <Card>
          <View style={{ alignItems: 'center', paddingVertical: 36 }}>
            ...
            <Text style={styles.emptyTitle}>No readings this period</Text>
            <Text style={styles.emptyText}>Daily readings give a much clearer picture of health.</Text>
          </View>
        </Card>
      ) : (
        report.healthLogs.map(log => (
          <Card key={log.log_type} style={styles.card}>
            <View style={styles.vitalHeader}>
              <Text style={{ fontSize: 22, marginRight: 8 }}>{healthIcons[log.log_type] || '📊'}</Text>
              <Text style={styles.vitalLabel}>
                {log.log_type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
              </Text>
              <View style={styles.countBadge}>
                <Text style={styles.countBadgeTxt}>{log.count} readings</Text>
              </View>
            </View>
            ...
            {(() => {
              ...
              const ca = Math.abs(va.regression.change_per_week);
              return (
                <View style={{ marginTop: 8, flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  <View style={[styles.trendBadge, { backgroundColor: tc + '18', borderColor: tc + '50' }]}>
                    <Text style={[styles.trendBadgeTxt, { color: tc }]}>{tl}</Text>
                  </View>
                  {va.clinical_status && (
                    <View style={[styles.zoneBadge, { backgroundColor: va.clinical_status.color + '18' }]}>
                      <Text style={[styles.zoneBadgeTxt, { color: va.clinical_status.color }]}>{va.clinical_status.label}</Text>
                    </View>
                  )}
                  {ca > 0 && va.regression.trend !== 'stable' && (
                    <Text style={{ fontSize: 11, color: tc }}>
                      ~{ca} {va.unit}/week
                    </Text>
                  )}
                </View>
            ...
            <View style={styles.vitalStats}>
              ...
              <View style={styles.vitalStat}><Text style={styles.vitalStatLabel}>Low</Text><Text style={[styles.vitalStatVal, { color: '#00b894' }]}>{log.min_value}</Text></View>
              <View style={styles.vitalStat}><Text style={styles.vitalStatLabel}>High</Text><Text style={[styles.vitalStatVal, { color: '#e17055' }]}>{log.max_value}</Text></View>
              <View style={styles.vitalStat}><Text style={styles.vitalStatLabel}>Logs</Text><Text style={[styles.vitalStatVal, { color: colors.primary }]}>{log.count}</Text></View>
            </View>
          </Card>
        ))
      )}
      ...
      {!report?.mood.length ? (
        <Card>
          <View style={{ alignItems: 'center', paddingVertical: 36 }}>
            ...
            <Text style={styles.emptyText}>Daily check-ins help track emotional wellbeing over time.</Text>
          </View>
        </Card>
      ) : (
        <>
          <Card style={styles.card}>
            <Text style={styles.cardTitle}>Mood this period</Text>
            <MoodDistribution moodData={report.mood} />
            <View style={{ marginTop: 14 }}>
              ...
              return (
                <View style={{ gap: 10 }}>
                  <View style={styles.wbRow}>
                    <Text style={styles.wbKey}>Most common mood</Text>
                  ...
                  {negMoodCount > 2 && (
                    <View style={[styles.alertPill, { marginTop: 2 }]}>
                      <Text style={styles.alertPillTxt}>
                        More social activity or a check-in call may help lift mood.
                      </Text>
                    </View>
                  )}
                </View>
    ...
    <>
      ...
      {!report?.risks.length ? (
        <Card style={{ alignItems: 'center', paddingVertical: 48 }}>
          ...
          <Text style={styles.emptyTitle}>No alerts this period</Text>
          <Text style={styles.emptyText}>All readings are within expected ranges.</Text>
        </Card>
      ) : (
        <>
          <Card style={styles.card}>
            ...
            <View style={styles.statsGrid3}>
              {[
                ...
                { label: 'Watch',    count: totalRisks - criticalRisks - dangerRisks, color: '#fdcb6e' },
              ].map(r => (
                <View key={r.label} style={[styles.statBox3, { backgroundColor: r.color + '10' }]}>
          ...
          {report.risks.map((risk, i) => {
            const sv = sev(risk.severity);
            return (
              <View key={i} style={[styles.riskCard, { borderLeftColor: sv.border, backgroundColor: sv.bg, marginBottom: 10 }]}>
      ...
      {!report?.activity.length ? (
        <Card>
          <View style={{ alignItems: 'center', paddingVertical: 36 }}>
            ...
            <Text style={styles.emptyText}>Activity will appear here once health data is logged.</Text>
          </View>
        </Card>
      ) : (
        <>
          <Text style={styles.secHeader}>Activity Log</Text>
          <Card style={styles.card}>
            {report.activity.map((item, i) => {
              const meta: Record<string, { icon: string; label: string; color: string }> = {
                ...
                medication_missed: { icon: '❌', label: 'Dose Missed',      color: '#ff7675' },
              };
            ...
            <Card style={styles.card}>
              ...
              {Object.entries(
                ...
                const meta: Record<string, { label: string; color: string }> = {
                  ...
                  medication_missed: { label: 'Doses Missed',      color: '#ff7675' },
                };
                const info = meta[type] || { label: type, color: '#636e72' };
                const maxC = Math.max(...Object.values(
                  report.activity.reduce<Record<string, number>>((acc, it) => {
                    acc[it.activity_type] = (acc[it.activity_type] || 0) + it.count;
                    return acc;
                  }, {})
                ));
                return <MiniBar key={type} value={count} max={maxC} color={info.color} label={info.label} />;
              })}
            </Card>
          )}
        </>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.dateControlsWrap}>{renderDateControls()}</View>

      {!loading && (criticalRisks > 0 || dangerRisks > 0) && (
        <View style={[styles.riskBanner, { backgroundColor: criticalRisks > 0 ? '#be123c' : '#c2410c' }]}>
          <Text style={styles.riskBannerText}>
            {criticalRisks > 0
              ? `${criticalRisks} critical alert${criticalRisks > 1 ? 's' : ''} — medical review needed`
              : `${dangerRisks} elevated reading${dangerRisks > 1 ? 's' : ''} — please review`}
          </Text>
        </View>
      )}
        ...
        {SECTIONS.map(sc => (
          <TouchableOpacity key={sc.key}
            ...
            <Text style={[styles.sectionTabText, activeSection === sc.key && { color: '#fff' }]}>
              {sc.key === 'risks' && report?.risks.length
                ? `Alerts (${report.risks.length})`
                : sc.label}
            </Text>
          </TouchableOpacity>
        ))}
...
const styles = StyleSheet.create({
  ...
  downloadBtn:        { backgroundColor: colors.primary, paddingHorizontal: 14, paddingVertical: 7, borderRadius: 8 },
  ...
  datePill:           { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 10, backgroundColor: '#f0f7ff', borderWidth: 1, borderColor: '#c7dff7' },
  ...
  riskBanner:         { padding: 11, alignItems: 'center' },
  riskBannerText:     { color: '#fff', fontWeight: '600', fontSize: 13, textAlign: 'center' },
  sectionTabBar:      { backgroundColor: colors.white, paddingHorizontal: 12, paddingVertical: 8, maxHeight: 50, borderBottomWidth: 1, borderBottomColor: colors.border },
  sectionTab:         { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20, borderWidth: 1, borderColor: colors.border, marginRight: 7 },
  ...
  // Section header — plain, clean
  secHeader:          { fontSize: 13, fontWeight: '700', color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 8, marginTop: 6 },

  cardTitle:          { fontSize: 14, fontWeight: '700', color: colors.textPrimary, marginBottom: 12 },
  emptyTitle:         { fontSize: 15, fontWeight: '700', color: colors.textPrimary, marginBottom: 5 },
  ...
  summaryKey:         { fontSize: 13, color: colors.textPrimary },
  summaryVal:         { fontSize: 13, color: colors.textSecondary },
  badge:              { paddingHorizontal: 12, paddingVertical: 4, borderRadius: 20, borderWidth: 1, alignSelf: 'center' },
  badgeText:          { fontSize: 12, fontWeight: '600' },
  bigStat:            { fontSize: 20, fontWeight: '800', marginBottom: 2 },
  bigStatSub:         { fontSize: 12, color: colors.textSecondary },
  ...
  moodTrack:          { flex: 1, height: 6, backgroundColor: '#f0f2f5', borderRadius: 3, overflow: 'hidden', marginHorizontal: 8 },
  ...
  alertPill:          { backgroundColor: '#fffbeb', borderRadius: 8, padding: 10, borderWidth: 1, borderColor: '#fde68a' },
  alertPillTxt:       { fontSize: 12, color: '#92400e', lineHeight: 17 },
  wbRow:              { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  wbKey:              { fontSize: 13, color: colors.textPrimary },
  wbVal:              { fontSize: 13, color: colors.textSecondary, fontWeight: '600' },
  riskCard:           { borderLeftWidth: 3, padding: 12, borderRadius: 10, marginBottom: 8 },
  sevTag:             { alignSelf: 'flex-start', paddingHorizontal: 9, paddingVertical: 3, borderRadius: 6, marginBottom: 5 },
  sevTagTxt:          { fontSize: 11, fontWeight: '700' },
  ...
  vitalLabel:         { fontSize: 14, fontWeight: '700', color: colors.textPrimary, flex: 1 },
  countBadge:         { backgroundColor: '#f0f7ff', paddingHorizontal: 10, paddingVertical: 3, borderRadius: 10 },
  countBadgeTxt:      { fontSize: 11, color: colors.primary, fontWeight: '600' },
  ...
  vitalStatVal:       { fontSize: 15, fontWeight: '700', color: colors.textPrimary },
  ...
  activityBadge:      { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 10 },
  dropdownToggle:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 10, paddingHorizontal: 12, backgroundColor: colors.background, borderRadius: 10, borderWidth: 1, borderColor: colors.border },
  dropdownToggleLabel:{ fontSize: 13, fontWeight: '600', color: colors.textPrimary },
  dropdownChevron:    { fontSize: 20, color: colors.textSecondary },
  modalOverlay:       { flex: 1, backgroundColor: 'rgba(0,0,0,0.45)', justifyContent: 'flex-end' },
  modalBox:           { backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, maxHeight: 400 },
  modalTitle:         { fontSize: 16, fontWeight: '700', color: colors.textPrimary, marginBottom: 16, textAlign: 'center' },
  ...
  pickerItemActiveText:{ color: '#fff', fontWeight: '700' },
